        ...
        VerticalSlideColorPicker.OnColorChangeListener
{
    ...
    private Imagnifier mImagnifier;
    ...
    private void setUpCameraOutputs(int width, int height) {
        ...
        try {
            ...
            if (!mImagnifier.isCameraBackFacing() && !mImagnifier.isCameraFrontFacing()) {
        ...
        try {
            for (String cameraId : manager.getCameraIdList()) {
                ...
                if (facing == CameraCharacteristics.LENS_FACING_FRONT){
                    mImagnifier.setFrontCameraId(cameraId);
                }
                else if (facing == CameraCharacteristics.LENS_FACING_BACK){
                    mImagnifier.setBackCameraId(cameraId);
                }
            }
            mImagnifier.setCameraFrontFacing();
            mCameraId = mImagnifier.getFrontCameraId();
        } catch (CameraAccessException e) {
            e.printStackTrace();
        }
    }
    ...
    private void toggleCameraDisplayOrientation(){
        if(mCameraId.equals(mImagnifier.getBackCameraId())){
            mCameraId = mImagnifier.getFrontCameraId();
            mImagnifier.setCameraFrontFacing();
            closeCamera();
            reopenCamera();
        ...
        else if(mCameraId.equals(mImagnifier.getFrontCameraId())){
            mCameraId = mImagnifier.getBackCameraId();
            mImagnifier.setCameraBackFacing();
            closeCamera();
            reopenCamera();
    ...
    private Bitmap rotateBitmap(Bitmap bitmap, int orientation) {
        ...
        try {
            if (mImagnifier.isCameraFrontFacing()) {
    ...
    public void onAttach(Context context) {
        ...
        try{
            mImagnifier = (Imagnifier) getActivity();
